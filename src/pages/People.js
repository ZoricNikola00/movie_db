...
const People = () => {
    ...
    const {data,isLoading,isError,error,refetch}=useQuery(['people'],()=>fetchData(`https://api.themoviedb.org/3/person/popular?api_key=${procces.env.TMDB_API_KEY}&language=en-US&page=${page}`),{onSuccess:()=>  window.scrollTo(0, 0)})
    useEffect(() => {
      refetch()
    }, [page])
    if(isLoading){
